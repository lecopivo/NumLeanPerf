...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < warmups; i++) {
    for (size_t k = 0; k < batch_size; k++) {
      const double *xs = inputs[(i * batch_size + k) % input_count];
      numleanperf_black_box_array(xs, n);
      checksum += float_array_sum_loop(xs, n);
    }
  }
  for (size_t i = 0; i < samples; i++) {
    uint64_t start = numleanperf_now_nanos();
    for (size_t k = 0; k < batch_size; k++) {
      const double *xs = inputs[((warmups + i) * batch_size + k) % input_count];
      numleanperf_black_box_array(xs, n);
      checksum += float_array_sum_loop(xs, n);
    }
